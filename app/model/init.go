...
func LastQueId(db *sqlx.DB) (que_id int) {
	...
	sql := `select mod(ifnull(count(que_id),0),150)+1 maxno from sale where year(doc_date) = year(CURDATE()) and month(doc_date) = month(CURDATE()) and day(doc_date) = day(CURDATE())`
	fmt.Println("Query = ", sql)
