...
func (s *SaleType) SearchSaleType(db *sqlx.DB) (saleType []*SaleType,err  error) {
	query := `SELECT sale_type_id, ifnull(sale_type_name,'') as sale_type_name, ifnull(sale_type_description,'') as sale_type_description FROM sale_type where is_cancel = 0 order by sale_type_id	`
